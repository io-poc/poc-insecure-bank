...
// IO Environment
def ioPOCId = 'io-poc36'
def ioProjectName = 'InsecureBank'
def ioWorkflowEngineVersion = '2022.4.1'
def ioServerURL = "https://io36.codedx.synopsys.com"
def ioRunAPI = "/api/ioiq/api/orchestration/runs/"

// SCM - GitHub
def gitHubPOCId = 'github-poc'
def gitHubOwner = 'io-poc'
def scmBranch = 'multibranch-test-2'
def scmRepoName = 'poc-insecure-bank'

// AST - Coverity
def coverityConfigName = 'coverity-poc21'
def coverityStream = 'InsecureBankStream'
def coverityTrialCredential = 'coverity-trial'

// AST - Black Duck
def blackDuckPOCId = 'blackduck-poc135'
def blackDuckProjectName = 'InsecureBank'
def blackDuckProjectVersion = '1.0.3'

// BTS Configuration
def jiraAssignee = 'neil'
def jiraConfigName = 'jira-poc36'
def jiraIssueQuery = 'resolution=Unresolved'
def jiraProjectKey = 'INSEC'
def jiraProjectName = 'INSEC'

// Code Dx Configuration
def codeDxConfigName = 'codedx-poc36'
...
pipeline {
    ...
    stages {
        ...
        stage('SAST') {
            ...
            environment {
                COVERITY_TRIAL_RESULTS_PASSPHRASE = credentials("${coverityTrialCredential}")
            }
            steps {
                echo 'Running SAST using Coverity'
                synopsysIO(connectors: [
                    coverity(
                        configName: coverityConfigName,
                        stream: coverityStream)]) {
        ...
        stage('SAST+M') {
            ...
            steps {
                ...
                script {
                    print('High risk score or significant code-change detected. Perform manual secure code-review.')
                }
            }
        }
